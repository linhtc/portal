var FMApp = angular.module('FMApp', ['ur.file', 'ngPostMessage']);
...
FMApp.controller('FileManagerCtr', ['$scope', '$http', '$location',
  function ($scope, $http, $location) {
    ...
    $scope.$root.$on('$messageIncoming', function (event, data){
        ...
        $scope.sendMessage({status: true, message: 'Hi'});
    });
    $scope.sendMessage = function (data){
        $scope.$emit('$messageOutgoing', data);
    };
    ...
    FM.clickFile = function (file) {
      if (file.folder) {
        ...
        // Leon added
        if(FM.parentFrame){
            console.log('send file to parent');
            $scope.sendMessage({status: true, file: file});
        } else{
